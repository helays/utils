...
func (this HttpClient) InitClient() (*http.Client, error) {
	...
	trans := &http.Transport{
		TLSClientConfig: &tls.Config{
			InsecureSkipVerify: this.InsecureSkipVerify, // client 不对https 证书进行校验
		},
		TLSHandshakeTimeout: tools.AutoTimeDuration(this.TLSHandshakeTimeout, time.Second), // tls 握手超时,
		DisableKeepAlives:   this.DisableKeepAlives,                                        // 是否禁用持久连接。当设为false时，支持HTTP keep-alive，允许在一个TCP连接上发送多个HTTP请求。
		DisableCompression:  this.DisableCompression,                                       // 是否禁用对响应内容的自动解压缩。设为false表示接受编码过的响应，并由客户端自动解码。
		// 主机连接池管理
		MaxIdleConns:        this.MaxIdleConns,                                         // 控制整个客户端的最大空闲连接数。值为0表示没有限制。
		MaxIdleConnsPerHost: this.MaxIdleConnsPerHost,                                  // 限制每个主机的最大空闲连接数。同样地，0表示没有限制。
		MaxConnsPerHost:     this.MaxConnsPerHost,                                      // 每个主机的最大连接数（包括活跃和空闲）。0表示无限制。
		IdleConnTimeout:     tools.AutoTimeDuration(this.IdleConnTimeout, time.Second), // 设置空闲连接在被关闭前等待新请求的时间长度。0表示不主动关闭空闲连接。
		//超时配置
		ResponseHeaderTimeout: tools.AutoTimeDuration(this.ResponseHeaderTimeout, time.Second), // 等待响应头的超时时间。0表示没有超时。
		ExpectContinueTimeout: tools.AutoTimeDuration(this.ExpectContinueTimeout, time.Second), // 在发送请求体之前等待服务器预检响应(100 Continue)的时间。0表示不等待。

		MaxResponseHeaderBytes: this.MaxResponseHeaderBytes, // 从服务器读取响应头的最大字节数。0表示没有限制。
		WriteBufferSize:        this.WriteBufferSize,        // 写缓冲区大小。0表示使用系统默认值。
		ReadBufferSize:         this.ReadBufferSize,         // 读缓冲区大小。0表示使用系统默认值。
		ForceAttemptHTTP2:      this.ForceAttemptHTTP2,      // 强制尝试使用HTTP/2。如果设置为true，即使服务端不明确支持HTTP/2，也会尝试升级。
	}
	switch this.ProxyType {
	case ProxyHttp, ProxyHttps:
		u, err := url.Parse(this.ProxyAddr)
		if err != nil {
			return nil, err
		}
		trans.Proxy = http.ProxyURL(u)
	case ProxySocks5:
		dialer, err := proxy.SOCKS5("tcp", this.ProxyAddr, nil, proxy.Direct)
		if err != nil {
			return nil, err
		}
